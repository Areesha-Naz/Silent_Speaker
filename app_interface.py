...
class SilentSpeakerApp(ctk.CTk):
    def __init__(self):
        ...
        self.history_data = [] # List to track history for easy deletion
    ...
    def update_ui(self):
        ...
        if ret:
            ...
            h, w, _ = frame.shape
            box_w, box_h = 220, 140
            color = (46, 182, 125) if self.is_recording else (139, 148, 158)
            cv2.rectangle(frame, (int(w/2 - box_w/2), int(h/2 - box_h/2)), 
                          (int(w/2 + box_w/2), int(h/2 + box_h/2)), color, 2)

            img = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            img_ctk = ctk.CTkImage(light_image=img, dark_image=img, size=(700, 380))
    ...
    def toggle_session(self):
        ...
        if self.is_recording:
            ...
            self.output_text.configure(text="HELLO WORLD", text_color=COLORS["text"])
            self.confidence_bar.set(0.9)
        else:
    ...
    def save_to_history(self):
        ...
        if "Begin" not in txt and "Waiting" not in txt:
    ...
    def delete_last_history(self):
        if self.history_data:
            self.history_data.pop() # Remove last item
            self.refresh_history_display()

    def clear_all_history(self):
        self.history_data = [] # Wipe list
        self.refresh_history_display()
